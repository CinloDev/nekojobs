...
export function WeeklyGoalWidget() {
  ...
  const currentWeekApps = useMemo(() => {
    ...
    return applications.filter(app => {
      ...
      return appDate >= startOfCurrentWeek;
    }).length;
  }, [applications]);

  const progressValue = Math.min((currentWeekApps / (profile?.weeklyGoal || 20)) * 100, 100);

  return (
    <WidgetCard title="Objetivo Semanal" contentClassName="flex-1 flex flex-col justify-center gap-md">
      <div className="flex items-end justify-between">
        <div className="space-y-1">
          <h3 className="text-heading-lg font-bold">{currentWeekApps} <span className="text-body-sm font-normal text-text-secondary">/ {profile?.weeklyGoal || 20}</span></h3>
